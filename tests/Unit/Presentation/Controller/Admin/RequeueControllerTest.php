...
class RequeueControllerTest extends TestCase
{
    ...
    private RequeueController $controller;

    protected function setUp(): void
    {
        $this->handler = $this->prophesize(RequeueHandlerInterface::class);
        $this->controller = new RequeueController(
            $this->handler->reveal()
        );
    }
    ...
    /** @test */
    public function it_can_requeue_failed_messages(): void
    {
    ...
    /** @test */
    public function it_will_respond_with_bad_request_when_requeue_failed(): void
    {
        $request = new Request(content: json_encode([
            'identifiers' => [1, 2],
        ]));
